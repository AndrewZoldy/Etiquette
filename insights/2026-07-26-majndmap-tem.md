...
Эмодзи у сцены — общая оценка сцены по данным эпохи «дно». Эмодзи у подтемы — вывод разбора этой подтемы. Там, где половины сцены живут по-разному, это отдельно проговорено в пояснениях к сводной таблице.

---

## 0. Главное в семи строках

1. 623 статьи распределены по 11 сценам крайне неравномерно: на «стол и еду» ушло 136 статей (21,8% канала), на «деньги» — 12 (1,9%).
2. И это перекос наоборот: «стол и еда» на дне даёт медиану 5 680 показов, «деньги» — 449 179. Самая большая сцена канала сегодня самая слабая, самая маленькая — самая сильная.
3. Обвал не универсален. В каждой сцене есть подтемы, которые после августа 2025 работают как раньше: ресторан, чай и кофе, гость в чужом доме, магазины, антиэтикет, слова о семье, «кто платит».
4. Разница всегда одна: спор о поступке живого человека против справки о предмете. На дне медиана «пользы» против медианы «спорной нормы»: стол 3 950 против 32 473, отношения 76 281 против 189 073, дом и семья 81 100 против 1 294 351, цифровое 30 496 против 697 655, работа 295 305 против 1 304 871.
5. 39 статей канала (6,3%) — это рассказы о самой Валентине и её мероприятиях: светская хроника (18), афиша «Светская жизнь Петербурга» (10), репортажи с выставок (6), отчёты о лекциях в сцене «работа» (5). Их медианы — от 10 209 до 62 017 показов при 88–178 дочитываниях. Самый большой мёртвый груз канала.
6. Одна цифра, которая описывает перекос целиком: 39 статей про Валентину и её события дают 10–62 тысячи показов при сотне дочитываний; 12 статей про деньги дают медиану 809 781 показ.
7. Заголовки не сломались — сломалась функция текста. Медиана CTR на дне выросла в 2–4 раза почти во всех сценах, а раздачи нет: читатель кликает, алгоритм не раздаёт справку.
...
Пояснения к таблице (три сцены помечены 🟡 именно потому, что внутри них живут две разные редакции):
- **Прочее** — живая половина работает как до спада («Антиэтикет», медиана дна 619 640; манифесты 80 279), мёртвая половина — 18 статей светской хроники с медианой 10 209 показов и 132 дочитываний. Сцена в целом 🟡: живое и мёртвое примерно поровну.
- **Гости и праздники** — половина «дом и визит» (18 статей) на дне даёт 247 687 показов и 31 790 дочитываний, то есть 🟢; календарно-подарочная половина (51 статья) — 12 008 и 551, то есть 🔴. Сцена в целом 🟡.
- **История и высший свет** — 67 592 на дне получены с учётом афиши; без неё оставшиеся 8 статей дают 205 524 показа и 8 266 дочитываний (🟢), а сама афиша — 62 017 при 120 дочитываниях (🔴). Сцена в целом 🟡.
- **Дом и семья** — 4 статьи в эпохе 1, медиана по такой выборке не считается. Статус 🟢 поставлен по эпохе «дно» (157 694 показа при 15 статьях).
- **Стол и еда** 🟡 — при медиане дна 5 680 внутри сцены живут ресторан (128 806) и чай с кофе (176 552), а также два сверхвыброса 8 594 800 и 2 012 885. Сцена не мертва, мертва её справочная половина.
...
- Файл `2026-07-26-temy-kandidaty.md` — список из 60 тем, собранный из пробелов по всем 11 сценам, с готовыми заголовками и указанием, какая статья канала подсказывает каждую тему; там же двадцать заголовков для первой волны и раздел «чего делать не надо».
